...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // Preserve eager BF16 score and scaled-score rounding. Softmax scale
        // is one because scaling has already happened here.
        #pragma unroll
        for (int i = 0; i < size(acc_s); ++i) {
            acc_s(i) = float(cutlass::bfloat16_t(float(cutlass::bfloat16_t(acc_s(i))) * 0.08838834764831845f));
        }
