extern "C" __global__ void minnow_prepare_qkv(const __nv_bfloat16* input,
  ...
  if (head < 20) {
    ...
    __syncthreads();
    const __nv_bfloat16* weight = head < 16 ? qweight : kweight;
    ...
    if (col < 64) {
      ...
      value = __hadd(__hmul(value, cos[token * 64 + col]), __hmul(partner, sin[token * 64 + col]));
    }
  }
