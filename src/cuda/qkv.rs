...
impl CustomOp3 for Prepare<'_> {
    ...
    fn cuda_fwd(
        ...
        if heads != 24
            ...
            || self.eps <= 0.
        {
            candle_core::bail!("invalid mini QKV layout or parameters");
        }
        ...
        let input = x
            .as_cuda_slice::<bf16>()?
            .slice(xl.start_offset()..xl.start_offset() + tokens * 24 * 128);
        let qw = q
            .as_cuda_slice::<bf16>()?
        ...
        let mut output = unsafe { dev.alloc::<bf16>(tokens * 24 * 128)? };
...
mod tests {
    ...
    fn qkv_fusion_matches_explicit_fp32_norm_and_bf16_rope_bitwise() -> Result<()> {
        ...
        for n in [32, 96, 512] {
            let x = Tensor::arange(0f32, (n * 24 * 128) as f32, &dev)?
                .affine(0.000123, -3.)?
                .sin()?
                .affine(8., -0.2)?
                .to_dtype(DType::BF16)?
            ...
            let q = rotate(normalize(x.narrow(1, 0, 16)?.transpose(0, 1)?, &qweight)?)?;
            let k = rotate(normalize(x.narrow(1, 16, 4)?.transpose(0, 1)?, &kweight)?)?;
            let v = x.narrow(1, 20, 4)?.transpose(0, 1)?;
